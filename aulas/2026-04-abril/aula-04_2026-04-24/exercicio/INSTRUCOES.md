...
3. Cole na pasta `src/` do seu projeto
...
Abra o arquivo `src/app.py` no seu editor.
...
2. Extrair nome, cpf, apartamento e email
...
def adicionar_morador():
    ...
    cursor.execute("""
        INSERT INTO moradores (nome, cpf, apartamento, email)
        VALUES (?, ?, ?, ?)
    """, (nome, cpf, apartamento, email))
...
Isto é uma **lista de listas**. Cada lista interna é um morador: `[id, nome, cpf, apartamento, email]`
...
- [ ] Copiei `app.py` para a pasta `src/`
- [ ] Rodei o servidor com `python src/app.py`
