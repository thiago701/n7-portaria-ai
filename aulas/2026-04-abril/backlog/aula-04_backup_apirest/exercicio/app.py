...
def conectar_banco():
    ...
    O banco 'portaria.db' deve estar em src/infra/database/portaria.db
    Se não existir, você pode criar rodando primeiro o exercício da Aula 02 (Criar Banco).

    Retorna: conexão com o banco de dados
    """
    # Definir caminho do banco de dados
    # Este código pega o diretório onde app.py está e procura pelo banco em src/infra/database/
    caminho_app = os.path.dirname(os.path.abspath(__file__))
    caminho_banco = os.path.join(caminho_app, '..', '..', '..', 'src', 'infra', 'database', 'portaria.db')
...
#
# INSTRUÇÃO:
# Este endpoint deve receber dados JSON (nome, cpf, numero_residencia, email)
# e inserir um novo morador no banco de dados.
#
# Siga os passos:
# 1. Use request.get_json() para pegar os dados enviados
# 2. Extraia: nome, cpf, numero_residencia, email
# 3. Conecte ao banco
