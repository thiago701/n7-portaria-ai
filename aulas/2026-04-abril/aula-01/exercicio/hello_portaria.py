...
# ============================================
# CAMADA: CORE — CONFIGURAÇÃO (src/core/config.py na vida real)
# ============================================
...
# ============================================
# CAMADA: CORE — MODELO (src/core/models/ na vida real)
# ============================================
...
class Morador:
    ...
    Attributes:
        ...
        numero_residencia: Número do numero_residencia (ex: "101", "302A")
...
# ============================================
# CAMADA: CORE — USE CASE (src/core/usercase/ na vida real)
# ============================================
...
class PortariaService:
    ...
    def cadastrar_morador(self, nome: str, numero_residencia: str, bloco: Optional[str] = None) -> Morador:
        """
        Cadastra um novo morador no sistema.

        Args:
            nome: Nome completo
            numero_residencia: Número do numero_residencia
        ...
        Raises:
            ValueError: Se nome estiver vazio ou numero_residencia duplicado
        """
        # Validação: nome não pode ser vazio
        if not nome.strip():
            raise ValueError("Nome do morador não pode ser vazio")

        # Validação: numero_residencia não pode ser duplicado no mesmo bloco
        for m in self._moradores:
            if m.numero_residencia == numero_residencia and m.bloco == bloco:
                raise ValueError(f"Já existe morador no Apto {numero_residencia}" +
                                 (f" Bloco {bloco}" if bloco else ""))

        # Cria e armazena o morador
        morador = Morador(nome=nome, numero_residencia=numero_residencia, bloco=bloco)
...
# ============================================
# CAMADA: INTERFACE (src/interface/ na vida real)
# ============================================
...
# - Pede ao usuário: nome (input), numero_residencia (input), bloco (input, pode ser vazio)
...
#     numero_residencia = input("Apartamento: ")
...
def main() -> None:
    ...
    print(f"\n{'=' * 50}")
    print("  Ademilson, cada camada do código tem