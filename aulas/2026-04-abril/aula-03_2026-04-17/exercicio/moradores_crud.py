...
def cadastrar_morador():
    ...
    try:
        ...
        # TODO 3: Peça o apartamento
        apartamento = input("Apartamento (ex: 101, 202): ").strip()
        ...
        cursor.execute(
            """INSERT INTO moradores
               (nome, cpf, apartamento, email, data_cadastro, data_atualizacao)
               VALUES (?, ?, ?, ?, ?, ?)""",
            (nome, cpf, apartamento, email, data_atual, data_atual)
        )
...
def listar_moradores():
    ...
    try:
        ...
        cursor.execute(
            "SELECT id, nome, cpf, apartamento, email FROM moradores WHERE ativo = 1 ORDER BY nome"
        )
        ...
        for morador in moradores:
            id_m, nome, cpf, apartamento, email = morador
            cpf_formatado = formatar_cpf(cpf)
            email_display = email if email else "Sem email"
            print(f"{id_m:<4} {nome:<25} {cpf_formatado:<14} {apartamento:<6} {email_display:<25}")
...
def buscar_morador():
    ...
    try:
        ...
        cursor.execute(
            "SELECT id, nome, cpf, apartamento, email, ativo FROM moradores WHERE nome LIKE ? AND ativo = 1",
            (f"%{nome_busca}%",)
        )
        ...
        for morador in resultados:
            id_m, nome, cpf, apartamento, email, ativo = morador
...
def atualizar_morador():
    ...
    try:
        ...
        novo_apartamento = input("Novo apartamento: ").strip() or morador[3]

        # Atualizamos no banco
        data_atual = obter_data_hora_atual()
        cursor.execute(
            """UPDATE moradores
               SET nome = ?, email = ?, apartamento = ?, data_atualizacao = ?
               WHERE id = ?""",
